...
case class SimpleComponent(p1: String, p2: Int) extends StatefullComponent[MyState] {
  ...
  def storeTo(e: TextEvent) = {
    //setState(new PathModify[MyState, Int]().setTo(e.input.value.toInt))
  ...
  override def view(): Element = {
    ...
    div()(
      input(defaultValue = RangeStore.to.toString).on(Input,this)(_.storeTo(_)),
      p()(
